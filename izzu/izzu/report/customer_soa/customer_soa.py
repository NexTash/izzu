# Copyright (c) 2025, nexyash and contributors
...
def execute(filters=None):
	...
	aging_row = get_aging_data(filters)

	return columns, res + [aging_row]
...
def get_gl_entries(filters, accounting_dimensions):
    ...
    if accounting_dimensions:
        ...
        select_fields += dimension_fields

    order_by_statement = "posting_date, account, creation"

    if filters.get("include_dimensions"):
        order_by_statement = "posting_date, creation"

    if filters.get("group_by") == "Group by Voucher":
        order_by_statement = "posting_date, voucher_type, voucher_no"
    elif filters.get("group_by") == "Group by Account":
        order_by_statement = "account, posting_date, creation"
    ...
    for gle in gl_entries:
        ...
        if gle["voucher_type"] == "Sales Invoice":
            si = frappe.db.get_value(
                ...
                ["docstatus", "is_return", "po_no", "remarks", "customer_name"],
                as_dict=True
            )
        ...
        party_name = p_name
        gle["party_name"] = party_name

        # Create doc_ref with details + voucher_no
        if details:
            gle["doc_ref"] = f"{details}"
        else:
            gle["doc_ref"] = ""

        voucher_type = gle.get("voucher_type")
        party = gle.get("party")
        unique_key = (voucher_type, gle["voucher_no"], party)
    ...
    if filters.get("from_date"):
        opening_balances = frappe.db.sql("""
            ...
            WHERE 
                ...
                AND company = %s
            GROUP BY party
        """, (filters["from_date"], filters["company"]), as_dict=True)

        for entry in opening_balances:
            entry["balance"] = flt(entry.opening_debit) - flt(entry.opening_credit)
    # frappe.msgprint(f"Opening Balance: {opening_balances}")
...
def get_aging_data(filters):
    ...
    # Handle cases where party is missing or empty list
    if not customer:
        return {}
    ...
    aging_buckets = [0, 0, 0, 0, 0]  # 0–30, 31–60, 61–90, 91–120, >120
    ...
    invoices = frappe.get_all(
        ...
        fields=["due_date", "posting_date", "outstanding_amount"],
    )

    for inv in invoices:
        entry_date = inv.due_date or inv.posting_date
        entry_date = getdate(entry_date)

        if entry_date > to_date:
            continue  # skip future dues

        age = (to_date - entry_date).days

        if 0 <= age <= 30:
            aging_buckets[0] += inv.outstanding_amount
        elif 31 <= age <= 60:
            aging_buckets[1] += inv.outstanding_amount
        elif 61 <= age <= 90:
            aging_buckets[2] += inv.outstanding_amount
        elif 91 <= age <= 120:
            aging_buckets[3] += inv.outstanding_amount
        elif age > 120:
            aging_buckets[4] += inv.outstanding_amount

    return {
        "date": "",
        "transactions": "",
        "details": "Aging Summary",
        "amount_dr": "",
        "amount_cr": "",
        "balance": "",
        "aging_0_30": format_currency(aging_buckets[0]),
        "aging_31_60": format_currency(aging_buckets[1]),
        "aging_61_90": format_currency(aging_buckets[2]),
        "aging_91_120": format_currency(aging_buckets[3]),
        "aging_over_120": format_currency(aging_buckets[4]),
    }
...
def get_totals_dict():
	...
	return _dict(
		opening=_get_debit_credit_dict(_("Opening Balance")),
