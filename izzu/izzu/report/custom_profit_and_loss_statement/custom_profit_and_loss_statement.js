...
frappe.provide("custom_profit_and_loss");
...
frappe.query_reports["Custom Profit and Loss Statement"] = $.extend({}, custom_profit_and_loss);


// frappe.query_reports["Custom Profit and Loss Statement"] = $.extend({}, emkan_ui.events.financial_statements);
...
function period_start_date(){
	let period_start_date = frappe.query_report.get_filter_value('period_start_date');
	if (period_start_date) {
		localStorage.setItem("period_start_date", JSON.stringify({period_start_date}));
	} else {
